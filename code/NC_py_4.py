...
for hs_idx, hidden_size in enumerate(hidden_sizes):
    for lr_idx, lr in enumerate(learning_rates):
        fold_metrics = {'f1': [], 'acc': [], 'recall': [], 'train_time': [], 'pred_time': [], 'val_loss': [], 'train_loss': []}
        for fold in range(5):
            ...
            fold_metrics['train_time'].append(result['train_time'])  # Fixed key
            fold_metrics['pred_time'].append(result['pred_time'])    # Fixed key
        ...
        grid_train_time[hs_idx, lr_idx] = np.mean(fold_metrics['train_time'])  # Correct matrix
        grid_predict_time[hs_idx, lr_idx] = np.mean(fold_metrics['pred_time'])  # Correct matrix
...
# Accuracy Heatmap (unchanged)
plt.subplot(1, 2, 1)
sns.heatmap(grid_acc, annot=True, xticklabels=learning_rates, yticklabels=hidden_sizes, cmap='viridis', fmt=".2f")
plt.title('Accuracy Heatmap')
plt.xlabel('Learning Rate')
plt.ylabel('Hidden Size')

# Training Time Heatmap (updated to show seconds)
plt.subplot(1, 2, 2)
sns.heatmap(
    grid_train_time, 
    annot=True, 
...
plt.title('Training Time Heatmap (Seconds)')  # Explicitly state units
plt.xlabel('Learning Rate')
plt.ylabel('Hidden Size')
