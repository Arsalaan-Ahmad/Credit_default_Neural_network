...
% Hyperparameters
hiddenSizes = [128,144,160,176,192];
%hiddenSizes = [ 128];    
%learningRates = [0.9];  
learningRates = [0.95,0.9,0.8,0.7];    
lambda_values = 0.001;          
epochs = 1000;                   
...
for hs = 1:length(hiddenSizes)
    for lr = 1:length(learningRates)
        ...
        fold_metrics = struct('f1',[], 'acc',[], 'time',[]);
        ...
        parfor fold = 1:cv.NumTestSets
            ...
            % Train model
            tic;
            [~, ~, valMetrics, time] = trainMLP(X(trainIdx,:), y(trainIdx),...
                X(testIdx,:), y(testIdx), hiddenSizes(hs), learningRates(lr),...
                epochs, lambda_values, class_weights);
            
            % Store metrics
            fold_metrics(fold).f1 = max(valMetrics.F1);
            fold_metrics(fold).acc = max(valMetrics.Accuracy);
...
% Final evaluation
[final_acc, final_f1, y_pred, y_prob] = evaluateModel(X, y, final_model.W1,...
    final_model.B1, final_model.W2, final_model.B2);

fprintf('\nFinal Performance:\n Accuracy: %.4f\n F1: %.4f\n Training Time: %.2fs\n',...
        final_acc, final_f1, sum(trainTime));
...
% Visualization Section
% Confusion Matrix (Now compatible types)
figure;
...
function [accuracy, F1, predictions, prob] = evaluateModel(X, y, W1, B1, W2, B2)
    batch_size = 1024;
